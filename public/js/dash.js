$(document).ready(function(){
    ...
    $('.contImgHover').click(async function(e){
        try {
            e.preventDefault()
            const { value: file } = Swal.fire({text: 'Elige tu nueva imagen de perfil',input: 'file',title: 'Imagen de perfil',inputAttributes:{'accept': 'image/*','multiple': false}})
            const formData = new FormData()
    ...
    $('body').on('click', '.wp-division .wp-content .leaf .wp-allArticles .wp-article .wp-actions .visibleArticleBtn', async function(e){
        try {
            ...
            const apiResponse = await axios({url:'/api/changeVisibleArticle',method:'PUT',data: {visible:stateToChange,id:changerKey}})
            Swal.fire({title: 'Bien Hecho',text: 'La visibilidad del articulo ha sido cambiada '})
    ...
    $('body').on('click','.wp-division .leaf .wp-files .card-file .wp-infoMan .wp-btnUse .clickerView',async function(e){
        try {
            ...
            const { value:visible } = await Swal.fire({title: 'Visibilidad del documento',text: 'Elije si este articulo sera visible o no',input: 'select',inputOptions:optionsSelect})
